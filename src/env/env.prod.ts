export const ENV = {
    // ENDPOINT: "http://localhost:8070/ventasOnline/rest",
    ENDPOINT_PRODUCT_SERVICE: "http://localhost:8084/producto-service",
    ENDPOINT_USER_SERVICE: "http://localhost:8084/tienda-service",
    ENDPOINT_OFFICE_SERVICE: "http://localhost:8084/tienda-service",
    ENDPOINT_CLIENT_SERVICE: "http://localhost:8084/tienda-service",
    ENDPOINT_INVENTORY_SERVICE: "http://localhost:8084/tienda-service",
    CURRENT_ENV: "dev"
}